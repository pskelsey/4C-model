...
2. Your risk model describes the impact of a climate variable on the density of pests per square meter of crop cover. To project the total number in each grid cell, select 'Yes' and set the density field equal to 1: projected number per cell = risk model value (no. m^-^2) x area of crop per cell x 1 (square.
3. Your risk model describes the impact of a climate variable on the density of lesions per square meter of crop canopy. To project the total number in each grid cell, select 'Yes' and set the density field equal to the ratio of canopy area to ground area (i.e. the leaf area index): projected number per cell = risk model value (no. per square meter) x area of crop per cell
4. Your risk model describes the impact of a climate variable on the density of an organism per observational unit (e.g. spores per lesion). To project the total number of spores in each grid cell, select 'Yes' and set the density field equal to the ratio of the observational unit to ground area (e.g. lesions per square meter of ground cover).
